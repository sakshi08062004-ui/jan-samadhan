...
async def get_admin_users():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, username, email, full_name, phone, address, role, created_at FROM users ORDER BY created_at DESC")
    users = []
    for row in c.fetchall():
        users.append({
            "id": row[0],
...
def get_user(username: str):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, username, email, password_hash, full_name, phone, address, role FROM users WHERE username = ?", (username,))
    user = c.fetchone()
    conn.close()
    return user
...
def get_user_by_id(user_id: int):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT id, username, email, password_hash, full_name, phone, address, role FROM users WHERE id = ?", (user_id,))
    user = c.fetchone()
    conn.close()
    return user
