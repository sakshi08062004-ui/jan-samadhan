...
def determine_points_level(points: int) -> str:
    if points >= 500:
        return "Visionary Citizen"
    if points >= 250:
...
def determine_points_badge(level: str) -> str:
    badge_map = {
        "Visionary Citizen": "Platinum",
        "Community Champion": "Gold",
        "Active Member": "Silver",
        "Community Participant": "Bronze"
    }
    return badge_map.get(level, "Bronze")


def collect_leaderboard_metrics(user_id: int, cursor) -> dict:
    cursor.execute("SELECT COUNT(*) FROM complaints WHERE user_id = ?", (user_id,))
    filed = cursor.fetchone()[0]
    cursor.execute("SELECT COUNT(*) FROM complaints WHERE user_id = ? AND status = 'resolved'", (user_id,))
...
async def get_user_points(user_id: int):
    ...
    total_points = user[8] or 0 # points is index 8 in our named SELECT
    level = determine_points_level(total_points)
    badge = determine_points_badge(level)
    return {"total_points": total_points, "level": level, "badge": badge}
...
@app.get("/api/points")
async def get_points(current_user: dict = Depends(verify_token)):
    """Get user points and level"""
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT points FROM users WHERE id = ?", (current_user["user_id"],))
    row = c.fetchone()
    conn.close()
    ...
    if row:
        ...
        badge = determine_points_badge(level)
        return {"total_points": total_points, "level": level, "badge": badge}
    return {"total_points": 0, "level": 1, "badge": "Newcomer"}
...
async def get_leaderboard():
    ...
    for row in c.fetchall():
        ...
        metrics = collect_leaderboard_metrics(user_id, c)
        level = determine_points_level(points or 0)
        badge = determine_points_badge(level)
        leaderboard.append({
            "user_id": user_id,
            "username": username,
...
async def create_complaint(complaint: ComplaintCreate, current_user: dict = Depends(verify_token)):
    ...
    # Award points
    points_awarded = 20
    c.execute("UPDATE users SET points = points + ? WHERE id = ?", (points_awarded, current_user["user_id"]))
    conn.commit()
    conn.close()
    
    return {"id": complaint_id, "points_awarded": points_awarded}
